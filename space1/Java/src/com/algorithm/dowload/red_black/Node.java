package com.algorithm.dowload.red_black;/*Node class*/
...
import java.util.*;
 ...
 public class Node implements ClassInterface{
   ...
   public void Show_ID(boolean b) {
	...
	public void Draw_Node( Graphics g, DrawingPanel dp ) {
	// public void Draw_Node( Graphics g ) {
		...
		System.out.println("Node: DrawNode: Inside DrawNode");

		value_font = new Font("Helvetica", Font.BOLD, 10);
		g.setFont(value_font);
		tx = (int)((x - radius)*x_scale);
		ty = (int)((y - radius)*y_scale);
		diameter = (int)(radius*(x_scale+y_scale));
  	g.setColor(node_colour);
		g.fillOval(tx,ty, diameter, diameter  );
		if (show_id) {
  ...
  public void Redraw_Node( Graphics g, DrawingPanel dp ) {
  // public void Redraw_Node( Graphics g ) {
		int tx, ty;
   	int diameter;

	System.out.println("Node: Redraw_Node: Inside redrawNode");
   ...
   public int Mid_X() {		return (x); }
    
	public int Mid_Y() { return (y); }
	...
	public int Get_ID( ) { return (node_id); }
	
	public void Set_Cost( double val ) {
		value = val;
		}
	
	public double Cost( ) { return value; }
  ...
  public void Show_Ans( Graphics g, DrawingPanel dp ) {
  // public void Show_Ans( Graphics g ) {
	...
	public void Unhighlight_Node(Graphics g, Color orig_colour) {
      int temp_x;
      int temp_y;
      int i;
      temp_x = x;
