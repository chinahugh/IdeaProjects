package com.algorithm.dowload.red_black;
...
class Arrow {
	...
	int start_x, start_y;
	int end_x, end_y;
	double img_mid_x, img_mid_y, img_cir;
	double arc_angle, start_angle, value;
	int num_arcs, mid_pt_x, mid_pt_y;
	...
	//method declaration
	public Arrow(Color value_colour, double value, int start_x, int start_y, int end_x, int end_y, double img_mid_x, double img_mid_y, double img_cir, double start_angle, double arc_angle, Color arc_colour, int num_arcs, int mid_pt_x, int mid_pt_y) {
	...
	public void Draw_Arrow( Graphics g, int testcase ) {
		double arrow_angle;
...
//		}
		...
		if (value != 0) {
			...
			Display_Value( g, testcase, arrow_x, arrow_y );
			}
	}
...
private void Display_Value( Graphics g, int testcase, int[] ax, int[] ay ) {
	int pos_x, pos_y;
