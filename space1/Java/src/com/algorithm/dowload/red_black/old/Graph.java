package com.algorithm.dowload.red_black.old;/* Graph class */
			...
			if (edges[i].Get_Start() == from && edges[i].Get_End() == to) {
				switch(val) {
					case 1: 
						...
						break;
					case 2:
						change_to = Color.yellow; //intermediate
						break;
					case 3:
...
*/	
	...
	public void Add_VNode( Node n ) {
		v_nodes.addElement((Object)n);
		//line added for testing purposes
		//System.out.println( "" + node_cnt ); 
		}
	
	public int VNode_Size() {
		return v_nodes.size();
		}
	...
	public void Draw_Graph( Graphics g, int height, int width ) {
	   ...
	   if (n_nodes != 0)	{
			...
			for( i = 0; i< node_cnt; i++ ) {
				nodes[i].Draw_Node( g );
				//System.out.println("draw node i = "+i);
				}
			for( i = 0; i< edge_cnt; i++ ) {
...
//System.out.println("num edge " + num);
