package com.algorithm.dowload.ins_sort;
...
public class DrawingPanel extends Panel {
    ...
    public void moveNodeRight(int i) {
	((Node)sortedNodes.elementAt(i)).x += 10;
	arrowX += 10;
	repaint(); delay();
	arrowX += 11;
	((Node)sortedNodes.elementAt(i)).x += 11;
	repaint(); delay();
    }
	...
	int dest_y = panel_height/2 - origNodesPosn + 100;
    ...
    public void flashNode(int i) {
	if (sortedNodes.size() > i) {
	    Node node = (Node)sortedNodes.elementAt(i);
	    arrowX = node.x + 10; arrowY = node.y - 20;
	    node.highlight = true;
	    repaint(); delay();
	    node.highlight = false;
	    repaint(); delay();
	...
	if (arrowX != -1) {
	    drawVertArrow(g, arrowX, arrowY, arrowX, arrowY + 19);
	}
	...
	int x = tableOffsetX - testCode.length() * 8; 
	int y = 5*panel_height/6;
	g.setFont( bigFont );
