package com.algorithm.dowload.dijkstra;/*Node class*/
...
import java.util.*;
 ...
 public class Node implements ClassInterface{
   ...
   int y;
   double diameter;
   Font ans = new Font("Helvetica", Font.BOLD, (int)(12*x_scale/y_scale));
	int node_id;
...
boolean showInfinite = false;
   ...
   public void Show_ID(boolean b) {
	...
	public void Draw_Node( Graphics g ) {
		...
		g.drawOval(tx, ty, (int)diameter, (int)diameter);
		if (show_id) {
			g.setColor(label_colour);
			g.drawString(""+ (char)('A' - 1 + node_id), tx, ty);
    		}
	}
   ...
   public void Show_Ans( Graphics g ) {
    ...
    public void drawInfinite( Graphics g ) {
		...
		g.drawString("o",  temp_x+(r3*2)/5, temp_y+(r3*8)/15);
		g.drawString("o",  temp_x+(r3*2)/5+6, temp_y+(r3*8)/15);
    }
