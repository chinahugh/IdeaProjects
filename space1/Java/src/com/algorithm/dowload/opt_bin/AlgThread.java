package com.algorithm.dowload.opt_bin;/* AlgThread.java */
...
public class AlgThread extends Thread {
     ...
     * in data menu.
     */
    public static String[] dataSets = {"Fixed Data Set"};
	...
	if (this.isAlive())
	    this.stop();
	drawingPanel.init();
	a = new OBSearch( data.length, data, label, drawingPanel, frame );
    }
...
//----
	...
	drawingPanel.init();
	a = new OBSearch( data.length, data, label, drawingPanel, frame );
	a.BuildOptTree( drawingPanel, frame );
	...
	frame.getSkipItem().enable();
	frame.getRunItem().enable();
	frame.getStopItem().disable();
