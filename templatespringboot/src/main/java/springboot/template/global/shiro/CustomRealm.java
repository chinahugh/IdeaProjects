...
public class CustomRealm extends AuthorizingRealm {
     ...
     */
//    @Override
    public void setCredentialsMatcherr(CredentialsMatcher credentialsMatcher) {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        if (principals == null) {
            logger.info("PrincipalCollection method argument cannot be null");
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        logger.info("JSONArray.fromObject(oneUser) = {}", oneUser);
        if (oneUser == null) {
            throw new UnknownAccountException("No account found for admin [" + username + "]");
        }
        if (oneUser.getIsDisable() == 2) {
            throw new ServiceException("账号还未通过审核!!!");
        }
        //查询用户的角色和权限存到SimpleAuthenticationInfo中，这样在其它地方
        //SecurityUtils.getSubject().getPrincipal()就能拿出用户的所有信息，包括角色和权限
        List<SysRole> roleList = sysRoleService.getUserRoles(oneUser.getId());
