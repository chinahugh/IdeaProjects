...
public class BarcodeZJReceiveController extends BaseController {
	private static String path = "/tmgl/barcodezjreceive/";// jsp路径
	// navTabId
	private static String navTabId = "barcodezjreceive";
	...
	/**
	 * 二维码管理首页 <br/>
	 * 页面：/tmgl/barcodezjreceive/list.jsp
	 */
	@Override
	public void index() {
		setAttr("page", getPageData());
		setAttr("fid", getParaToInt());
		System.out.println("fidd" + getParaToInt());
	...
	private Page<BarcodeReceive> getPageData() {
		String sql = " from barcodeReceive where tag=2 and fid= " + getPara();
	...
	/**
	 * 添加二维码 <br/>
	 * 页面：/tmgl/barcodezjreceive/add.jsp
	 */
	public void add() {
		setAttr("fid", getPara());
		render(path + "add.jsp");
