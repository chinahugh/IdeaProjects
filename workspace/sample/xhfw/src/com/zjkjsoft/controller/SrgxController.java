...
import com.zjkjsoft.model.Srgx;
...
public class SrgxController extends BaseController {
	private static String path="/xtgl/rolgx/";//jsp路径
	//navTabId
	private static String navTabId="srgx";
	@Override
	...
	@Override
	public void index() {
		setAttr("treeJSStr", PageElement.getTreeC1("select * from srole where  id>? order by xh ,id ", Integer.valueOf(0), "srgx/list/","srgxBox", "mc"));
	 ...
	 */
	public void list() {
		List<Record> list = Db.find("select * from srgx where  id="+getParaToInt() +" order by id ");
	...
	public void save() {
		...
		try{
			Srgx m=getModel(Srgx.class);
			int id=m.get("id");
			Srgx.dao.deleteById(id);
