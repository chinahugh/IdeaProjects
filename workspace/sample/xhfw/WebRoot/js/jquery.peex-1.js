(function ($) {
    ...
    $.mask = {
        load: function (b, c) {
            ...
            if (b.closeOnEsc) {
                $(document).bind("keydown-1.mask.htm"/*tpa=http://www.ankang.gov.cn/JS/keydown.mask*/, function (e) {
                    if (e.keyCode == 27) {
                        $.mask.close(e)
                    }
                })
            ...
            if (b.closeOnClick) {
                m.bind("click-1.mask.htm"/*tpa=http://www.ankang.gov.cn/JS/click.mask*/, function (e) {
                    $.mask.close(e)
                })
            }
            $(window).bind("resize-1.mask.htm"/*tpa=http://www.ankang.gov.cn/JS/resize.mask*/, function () {
                $.mask.fit()
            });
            if (c && c.length) {
        ...
        close: function () {
            if (loaded) {
                ...
                $(document).unbind("keydown-1.mask.htm"/*tpa=http://www.ankang.gov.cn/JS/keydown.mask*/);
                m.unbind("click-1.mask.htm"/*tpa=http://www.ankang.gov.cn/JS/click.mask*/);
                $(window).unbind("resize-1.mask.htm"/*tpa=http://www.ankang.gov.cn/JS/resize.mask*/);
                loaded = false
            }
            return this
        },
