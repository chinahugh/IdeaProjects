(function ($) {
    ...
    $.mask = {
        load: function (b, c) {
            ...
            if (b.closeOnEsc) {
                $(document).bind("keydown.mask.htm"/*tpa=http://www.ankang.gov.cn/js/keydown.mask*/, function (e) {
                    if (e.keyCode == 27) {
                        $.mask.close(e)
                    }
                })
            ...
            if (b.closeOnClick) {
                m.bind("click.mask.htm"/*tpa=http://www.ankang.gov.cn/js/click.mask*/, function (e) {
                    $.mask.close(e)
                })
            }
            $(window).bind("resize.mask.htm"/*tpa=http://www.ankang.gov.cn/js/resize.mask*/, function () {
                $.mask.fit()
            });
            if (c && c.length) {
        ...
        close: function () {
            if (loaded) {
                ...
                $(document).unbind("keydown.mask.htm"/*tpa=http://www.ankang.gov.cn/js/keydown.mask*/);
                m.unbind("click.mask.htm"/*tpa=http://www.ankang.gov.cn/js/click.mask*/);
                $(window).unbind("resize.mask.htm"/*tpa=http://www.ankang.gov.cn/js/resize.mask*/);
                loaded = false
            }
            return this
        },
