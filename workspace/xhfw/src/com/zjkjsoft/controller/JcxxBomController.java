...
public class JcxxBomController extends BaseController {
	...
	private static String navTabId1="jcxxbomBbh";
	...
	public void index1() {
		...
		Product product=Product.dao.findById(probbh.getInt("fid"));
		setAttr("wlh", product.get("wlh"));
		setAttr("wlms", product.get("wlms"));
		setAttr("fid", product.get("id"));
		setAttr("bbh", probbh.get("bbh"));
		String wlh=getPara("wlh");
		if (wlh!=null&&!"".equals(wlh)) {
			setAttr("tree", JcxxBom.searchTree(probbh, wlh));
		} else {
			setAttr("tree", PageElement.getTreeBom("select wlh,wlms,zj,zjms,bbh,fid from jcxxbom where fid="+probbh.get("fid")+" and bbh='"+probbh.get("bbh")+"' and wlh=? order by id",product.getStr("wlh"), "jcxxbom/list/", "jcxxbomBox1","zj","bbh","zjms"));
		}
		render(path+"index1.jsp");
	}
	...
	private Page<Product> getPageData1()
	{
		...
		if(wlms!=null&&!"".equals(wlms))
			sql+=" and p.wlms like '%"+wlms+"%' ";	
		String wlh1=getPara("wlh");
		if(wlh1!=null&&!"".equals(wlh1))
			sql+=" and p.wlh like '%"+wlh1+"%' ";		
	...
	public void save() {
		...
		try{
			...
			if(b.get("id")!=null){
				b.update(); 
				toDwzJson(200,"修改操作成功",navTabId1,"","closeCurrent");
			}else{
				...
				if (jcxxbom!=null) {
					toDwzJson(200,"不能添加该组件",navTabId1);
				}else {
					b.save();
					toDwzJson(200,"新增操作成功",navTabId1,"","closeCurrent");
				}
			}
		}catch(Exception e){
			e.printStackTrace();
