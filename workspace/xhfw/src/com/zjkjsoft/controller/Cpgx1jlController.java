...
public class Cpgx1jlController extends BaseController {
	private static String path="/scxx/cpgx1jl/";//jsp路径
	//navTabId
	private static String navTabId="cpgx1jl";
	...
	private Page<Cpjcjl> getPageData()
	{
		...
		String sql="from cpjcjl a, cpjcjd b where a.fid=b.fid and a.lx=b.lx and a.xh=b.xh and  a.lx=1  ";
		//管理员  1
		//部门领导
		//操作者
	...
	{
		String keywords=getPara("keywords");
		String sql="from cpjcjl where lx=1  ";
		//管理员  1
		//部门领导
		//操作者
		if("管理员".equals(getSessionAttr("username"))) {
	 ...
	 */
	public void view() {
		String sql=" select a.*,b.mc from cpjcjl a, cpjcjd b where a.fid=b.fid and a.lx=b.lx and a.xh=b.xh and  a.lx=1 and a.id=? ";
	...
	public void addlx() {
		...
		//序号为1的节点及检测项目
			...
			List<Cpjcxm> xmL= Cpjcxm.dao.find("select a.* ,c.id   xid  from cpjcxm a,cpjcjd b , product c " + 
					"where a. fid=b.id and b.fid=c.id  and b.lx=1 and b.xh=1  and  c.wlh=?",wlh);
			setAttr("xmL", xmL);
			cpjc=new Cpjc();
			cpjc.set("txm",txm);
			setAttr("cpjc", cpjc);
...
//				toDwzJson(200,"新增操作成功",navTabId,"","closeCurrent");
				...
				if(cpjc.get("id")==null) {
					cpjc=new Cpjc();
					cpjc.set("txm", m.getStr("txm")).set("fid", m.getInt("fid")).set("state", 1).save();
				}
				if(xh<x) {
				  // m.set("lx", 2)
...
toDwzJson(200,"新增操作成功",navTabId,"","closeCurrent");
	...
	public void export() throws UnsupportedEncodingException {
		...
		String filename = new String("产品加工.xls".getBytes("UTF-8"), "ISO8859-1");
