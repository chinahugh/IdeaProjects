...
public class CpjcxmController extends BaseController {
	...
	@Override
	public void index() {	
		setAttr("treeJSStr", Cpjcjd.getTree3("cpjcxm/list/","cpgxjdBox", "mc"));				
	 ...
	 */
	public void add() {
		setAttr("cpjcjd", Cpjcjd.dao.findFirst("select id,lx   from cpjcjd  where id=?",getParaToInt()));
		render(path+"add.jsp");
		//setAttr("fid", getParaToInt());
		//render(path+"add.jsp");
	}
...
toDwzJson(200,"新增操作成功",navTabId,"","closeCurrent");
	 ...
	 */
	public void edit() {		
		setAttr("cpjcxm", Cpjcxm.dao.findFirst("select a.* ,b.lx  flx  from cpjcxm a, cpjcjd b where a.fid =b.id and a.id=?",getParaToInt()));
		render(path+"add.jsp");
	}
