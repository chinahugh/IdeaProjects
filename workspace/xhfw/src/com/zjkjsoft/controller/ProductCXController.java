...
public class ProductCXController extends BaseController {
	
	private static String path="/scxx/cpgxjl/";//jsp路径
	//navTabId
	private static String navTabId="cpgxjl";
	...
	private Page<Cpjc> getPageData()
	{
		...
		String sql="from cpjc a, cpjcjd b,probbh c  where  c.id=b.fid and  a.fid=b.fid and a.lx=b.lx and a.xh=b.xh     ";
		//管理员  1
		//部门领导
		//操作者
...
//		}
		...
		return  Cpjc.dao.paginate(getParaToInt("pageNum", 1),
				getParaToInt("numPerPage", 20), "  select a.*,b.mc  ,c.mc pmc ", sql+" order by id desc");
	}
	private Page<Cpjcjl> getPageDataw()
	{
		String keywords=getPara("keywords");
		String sql="from cpjcjl a, cpjcjd b where a.fid=b.fid and a.lx=b.lx and a.xh=b.xh and a.txm=(select txm from cpjc where id="+getParaToInt()+")";
		//管理员  1
		//部门领导
		//操作者
//		if("管理员".equals(getSessionAttr("username"))) {
	...
	public void addlx() {
		...
		//根据检测项目节点及检测项目
			...
			List<Cpjcxm> xmL= Cpjcxm.dao.find("select * from cpjcxm  where "
					+ "fid=(select id from cpjcjd  where lx=3 and fid=? and xh=?)",jl.getInt("fid"),jl.getInt("xh"));
			setAttr("xmL", xmL);
		}else {
		//no 根据条形码找物料码
...
toDwzJson(200,"修改操作成功",navTabId,"","closeCurrent");
				...
				int xh=m.getInt("xh");
				long x=Db.queryInt("select max(xh) from cpjcjd  where fid=? and lx=3",m.getInt("fid"));
...
//				toDwzJson(200,"新增操作成功",navTabId,"","closeCurrent");
				...
				if(xh<x) {
				  ...
				  // cpjc.set("lx", 2).set("xh", xh);
					...
					//m.set("xh", 1).set("lx", 3);
					cpjc.set("xh", 1).set("lx", 4);
				}
				m.set("lx", 3).set("state", 1).save();
...
toDwzJson(200,"新增操作成功",navTabId,"","closeCurrent");
	...
	public void export() throws UnsupportedEncodingException {
		List<Cpjc> data=getPageData().getList();
