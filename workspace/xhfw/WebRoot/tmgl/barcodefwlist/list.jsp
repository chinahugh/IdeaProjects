...
<div class="pageHeader">
	<form id="pagerForm" method="post"
		...
		<input type="hidden" name="status" value="${param.status}"> <input
			type="hidden" name="pageNum" value="${page.pageNumber }" /> <input
			type="hidden" name="numPerPage" value="${page.pageSize }" /> <input
			type="hidden" name="orderField" value="${param.orderField}" />
		<div class="searchBar">
			<table class="searchContent">
				<tr>
					<td>条码：<input type="text" name="tm" value="${param.tm}"
						size="20" />
					</td>
					<td>生成日期：<input class="required date" type="text" name="scrq"
						value="${param.scrq}" size="20" />
					</td>
					<td>条码方式：<input type="text" name="tmfs" value="${param.tmfs}"
						size="20" />
					</td>
					<td>打印状态：
...
<div class="pageContent">
	<form method="post" action="barcodefwlist/printconf" rel="ids"
		name="ids" onsubmit="return navTabSearch(this);">
		<div class="panelBar">
			<ul class="toolBar">
				<li><a class="edit" href="barcodefwlist/view/{sid_user}"
					target="navTab"><span>查看</span></a></li>
				<li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids"
					name="ids" href="barcodefwlist/delete" class="delete"><span>批量删除</span></a></li>
			</ul>
		</div>
		<div class="formBar">
			<ul>
			...
			<tbody>
				...
				<c:forEach items="${page.list}" var="o">
					<tr target="sid_user" rel="${o.id}"
						ondblclick="view('barcodefwlist/confprint/${o.id}')"
