 ...
 * @Author: hugh
 * @Time: 2018/01/11 5:43 PM
 * @version 1.0
 */
public class Demo1 {
    //初始化数组变量
    private char[] cNum = {'1','2','3','4','5','6','7','8','9','0'};
    private char[] cStr = {'a','b','c','d','e','f','g','h',
            'i','j','k','l','m','n','o','p',
            'q','r','s','t','u','v','w','x','y','z'};
    private int[] iMonth = {31,28,31,30,31,30,31,31,30,31,30,31};
     ...
     */
    private boolean isMail(String sPara){
        for (String aSMail : sMail) {
            if (sPara.indexOf(aSMail) == -1)
                return false;
        }
        return true;
    }
     ...
     */
    private boolean isNumber(String sPara){
     ...
     */
    private boolean isString(String sPara){
        int iPLength = sPara.length();
        for(int i=0;i<iPLength;i++){
            char cTemp = sPara.charAt(i);
            boolean bTemp = false;
     ...
     */
    private boolean chickDay(int iPara){
     ...
     */
    private int chickData(String sPara){
        boolean bTemp = false;
        //所输入日期长度不正确
